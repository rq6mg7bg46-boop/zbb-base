...
class WechatAutomation {
  ...
  async openAndEnter(): Promise<void> {
    ...
    try {
      ...
      // 步骤10：截图并使用 OCR 查找"绿城云"
      this.currentPhase = 'screenshot_ocr';
      automationEngine.log('info', '[微信] ⑩: 截图并使用 OCR 查找"绿城云"...');
      
      // 使用 findTextByMLKit 方法（AccessibilityServiceImpl 中已实现）
      const findResult = await ZBBAutomation.findTextByMLKit('绿城云');
      
      if (findResult?.found && findResult.centerX && findResult.centerY) {
        automationEngine.log('success', `[微信] ⑩: OCR 找到"绿城云" @ (${findResult.centerX}, ${findResult.centerY})`);
        
        // 步骤11：点击"绿城云"
        await this.delay('normal');
        this.currentPhase = 'click_xiaochengxu';
        await ZBBAutomation.click(findResult.centerX, findResult.centerY);
        automationEngine.log('success', '[微信] ⑪: 点击"绿城云"进入小程序');
      } else {
        automationEngine.log('error', `[微信] ⑩: 未找到"绿城云"，请检查屏幕`);
      ...
      automationEngine.log('success', '[微信] ========== 已进入绿城云小程序 ==========');
