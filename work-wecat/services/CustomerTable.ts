...
class CustomerTable {
   ...
   * 添加客户记录
   */
  async addRecord(rawData: string): Promise<CustomerRecord | null> {
    // 解析原始数据
  ...
  private parseCustomerData(rawData: string): { surname: string; gender: string; phone: string } | null {
    ...
    // 检查是否包含性别关键字
    if (beforePhone.includes('女士') || beforePhone.includes('小姐')) {
      gender = '女士';
      surname = beforePhone.replace(/女士|小姐/g, '').trim();
    } else if (beforePhone.includes('先生')) {
      surname = beforePhone.replace('先生', '').trim();
    } else {
      // 默认取第一个汉字作为姓氏
      const hanziMatch = beforePhone.match(/[\u4e00-\u9fa5]/);
      surname = hanziMatch ? hanziMatch[0] : beforePhone;
    }
    ...
    if (this.records.length === 0) {
      ...
      for (const record of this.records) {
        console.log(
          ...
          record.surname.padEnd(4) + ' | ' +
          record.gender.padEnd(4) + ' | ' +
