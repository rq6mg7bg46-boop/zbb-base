...
class DouyinAutomation {
  ...
  async fetchCustomerInfo(): Promise<CustomerInfo | null> {
    ...
    try {
      ...
      const foundName = await this.simulateOCR(this.friendName);
      if (foundName) {
        automationEngine.log('success', `[抖音] ③: 识别好友 "${foundName}"`);
      } else {
      ...
      await this.simulateClick(`好友"${foundName}"的对话框`);
      automationEngine.log('success', '[抖音] ④: 点击好友对话框');
