...
const STEP_CONFIGS: StepConfig[] = [
  {
    ...
    title: '校准步骤1',
    instruction: '请返回手机桌面',
    subInstruction: '点击屏幕上的"绿城云"小程序图标',
    targetApp: '桌面',
  },
  {
    step: 2,
  ...
  const handleClickRecorded = useCallback(async (coords: { x: number; y: number }) => {
    try {
      ...
      if (currentStep === 1) {
        ...
        Alert.alert(
          '校准完成！',
          `已保存的坐标:\n\n绿城云: (${Math.round(coords.x)}, ${Math.round(coords.y)})\n\n可以返回首页启动自动化流程了。`,
          [
            { text: '确定', onPress: () => {} },
          ]
        );
              ...
              <View style={[styles.coordCard, { backgroundColor: theme.backgroundDefault }]}>
                <View style={styles.coordRow}>
                  ...
                  <View style={styles.coordInfo}>
                    <Text style={[styles.coordLabel, { color: theme.textMuted }]}>绿城云图标</Text>
