...
/**
 * findCustomerStep - 千机步骤 3
 * 子流程：
 *   1. 找"报备审核"节点（最多 3 次滑动）
 *   2. 找"保利"判断界面（无则 Toast + 震动 + lastExitReason='no_baoli'）
 *   3. 转发 3 步：3-1 列表"转发" → 3-2 联系人"转发" → 3-3 分享页"复制"
 *   4. 解析 lastTextNodes → customerInfo
 */
export const findCustomerStep: StepFn<QianjiContext, void> = async (ctx) => {
  ...
  logToBoth(
    'info',
    `[千机：步骤3-1] 点击第1个"转发" @ (${firstForward.centerX}, ${firstForward.centerY})`
  );
  ...
  nodes2.forEach((node, index: number) => {
    if (node.text && node.text.trim().length > 0) {
      logToBoth(
        'info',
        `[千机：步骤3-2] 节点${index}: "${node.text}" @ (${Math.round(node.centerX)}, ${Math.round(node.centerY)})`
      );
    }
  });
  ...
  logToBoth(
    'info',
    `[千机：步骤3-2] 点击Y值最大的"转发" @ (${forwardInList.centerX}, ${forwardInList.centerY})`
  );
  // P+ 拟人化：关键 tap 前迟疑 + ±5px 偏移
  await maybePause();
  ...
  // 改用 ctx.lastTextNodes（步骤2 抓的"报备审核"页节点）
  const nodeText = assembleKeyValueLines(ctx.lastTextNodes);
  logToBoth(
    'info',
    `[千机：步骤3-4] 节点拼装后(${ctx.lastTextNodes.length}个原始节点):\n${nodeText.substring(0, 800)}`
  );
  if (nodeText.trim()) {
    ...
    if (parsed) {
      ...
      logToBoth(
        'info',
        `[千机：步骤3-4] 解析结果: 客户=${ctx.customerInfo!.customerName || '(空)'} 电话=${ctx.customerInfo!.phone || '(空)'} 经纪人=${ctx.customerInfo!.agent || '(空)'} 经纪人电话=${ctx.customerInfo!.agentPhone || '(空)'} 城市=${ctx.customerInfo!.city || '(空)'} 报备时间=${ctx.customerInfo!.reportTime || '(空)'}`
      );
    } else {
